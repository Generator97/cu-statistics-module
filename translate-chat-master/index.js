...
module.exports = function TranslateChat(mod) {
    ...
    mod.hook('S_CHAT', 3, {order: 100}, (event) => {
        ...
        getTranslation(event, mod.settings.targetLang, function(query) {
            if (query != undefined) {
                mod.send('S_CHAT', 3, Object.assign({}, event, {message: query.translation, authorName: event.authorName + ' (Translated)'}));
            }
        });
    });   
    ...
    mod.hook('S_WHISPER', 3, {order: 100}, (event) => {
        ...
        getTranslation(event, mod.settings.targetLang, function(query) {
            if (query != undefined) {
                mod.send('S_WHISPER', 3, Object.assign({}, event, {message: query.translation, authorName: event.authorName + ' (Translated)'}));
            }
        });        
    });    
